...
> **重要提醒**
> - 需要安装 openclaw-weixin 并正确连接成功后，才能使用 iLinkBridge 替换消息处理路径。
> - 启动 iLinkBridge 后，如果没有停用 openclaw-weixin channel，则会收到两条回复。
> - 另外还需要修改 openclaw.json 中的缓存reset机制（默认每天4点自动过期），配置方式见下文。
